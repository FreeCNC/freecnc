        ...
        CBP_LookUp(0), VPT_Table(0), offsets(0), sndbuf(0)
{
    ...
    sndbuf = new Uint8[2 * SOUND_MAX_UNCOMPRESSED_SIZE];
...
void VQAMovie::AudioHook(void* udata, Uint8* stream, int len)
{
...
Uint32 VQAMovie::DecodeSNDChunk(Uint8 *outbuf)
{
    ...
    if ((chunkid & vqa_t_mask) != vqa_snd_id) {
        logger->error("VQA: Decoding SND chunk - Expected 0x%X, got 0x%X\n",
...
bool VQAMovie::DecodeVQFRChunk(SDL_Surface *frame)
{
    ...
    if (chunkid != vqa_vqfr_id) {
        logger->error("VQA: Decoding VQFR Chunk - Expected 0x%X, got 0x%X\n",
                vqa_vqfr_id, chunkid);
        return false;
    }
...
inline void VQAMovie::DecodeUnknownChunk()
{
    ...
    vqafile->readDWord(&chunklen, 1);
    chunklen = SDL_Swap32(chunklen);
    logger->debug("Unknown chunk at %0x for %i bytes.\n", vqafile->tell(), chunklen);
